...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.button_white').forEach((btn, i) => {
        btn.onclick = (e) => {
            e.preventDefault();
            if (i === 0) { toggleModal(hours, true); highlightDay(); }
            else if (i === 1) document.querySelector('.hero_blur_small')?.scrollIntoView({behavior:'smooth'});
            else if (i === 2) { toggleModal(gallery, true); setTimeout(() => gallery?.querySelector('.vertical-gallery')?.scrollTo(0,0), 100); }
        };
    });
    ...
    document.querySelectorAll('.modal').forEach(m => {
        m.onclick = (e) => e.target === m || e.target.classList.contains('modal__close') ? toggleModal(m, false) : null;
    });

    document.onkeydown = (e) => e.key === 'Escape' && document.querySelectorAll('.modal.show').forEach(m => toggleModal(m, false));
});
